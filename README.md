...
### v1.5 (2026-03-13)
- ⚡ **性能优化** - 批量加载速度提升 20%（BATCH_SIZE: 10→15, BATCH_DELAY: 100ms→50ms）
- 🐛 **稳定性增强** - 为分红检测添加错误处理，单个基金出错不影响其他基金
- 🔧 **内存优化** - 修复 Canvas 事件监听器泄漏问题
- 💾 **数据安全** - 走势数据保存防抖优化（1000ms→300ms），页面关闭前强制保存
- 📐 **代码质量** - 提取配置常量，添加结构化注释，删除重复代码
- 🎯 **代码重构** - 提取表格渲染辅助函数，提升可维护性
