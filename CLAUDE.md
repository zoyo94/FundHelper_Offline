...
每天首次打开 popup 时运行：
1. 比较 `savedPrevDate` 与今天日期
2. 如果是新的一天 → 计算昨日收益
3. 更新 `yesterdayProfit` 和 `savedPrevPrice`
4. 处理已确认的待处理调整（T+1/T+2）
...
尽管是单个 3400 行文件，`popup.js` 有清晰的分区：

1. **全局状态**（第 1-10 行）
2. **工具函数**（storage、格式化、日期）
3. **Toast/Modal 系统**
4. **结算逻辑**（自动/手动/撤销）
5. **走势数据持久化**
6. **API 获取**（天天基金、新浪、腾讯）
