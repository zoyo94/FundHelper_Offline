...
- `formatDateTimeForFile(date)` - 文件名时间格式 YYYYMMDD_HHMM

### 代码简化
- 统一使用 `round6()` 替代 `parseFloat((value).toFixed(6))`
- 统一使用 `formatProfit()` 替代重复的正负号格式化代码
- 统一使用 `formatTime()` 替代重复的时间格式化代码

### 分红逻辑修复
- **修复**：调整执行顺序，分红检测在自动结算之前
- **原因**：避免分红导致的净值下跌被误判为亏损
- **影响**：确保 `holdProfit` 和 `yesterdayProfit` 计算正确
